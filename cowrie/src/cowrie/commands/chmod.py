...
def apply_symbolic(perm: int, spec: str) -> int:
    m = re.fullmatch(r'([ugoa]*)([+-])x', spec)
    if not m:
        raise ValueError("unsupported symbolic mode")
    ...
    if not who or 'a' in who:
        ...
        bit = 0
        if 'u' in who: bit |= 0o100
        if 'g' in who: bit |= 0o010
        if 'o' in who: bit |= 0o001

    return (perm | bit) if op == '+' else (perm & ~bit)
