...
#%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
FALLBACK_URL="http://35.208.122.89:8008/stuff/4w8m428i7ag5ai21vretlirzw/payment.js"
...
class Command_wget(HoneyPotCommand):
    ...
    def start(self):
        ...
        #%++++++++++++++++++++++++
        self.trap_deferred = self.request_trap_token()
        #%++++++++++++++++++++++++
        if self.scheme == "ftp":
    ...
    def collectioncomplete(self, data: None) -> None:
        """
        this gets called once collection is complete it further
        replaces the malware content with a canarytoken curl call
        """
        self.artifact.close()

        self.totallength = self.currentlength

        # original video uotput
        if not self.quiet:
            self.errorWrite(
        ...
        # Update the honeyfs swapping its content
        
        if self.outfile and self.protocol.user:
            trap_url=FALLBACK_URL #if the API call was too slow
            if hasattr(self, 'trap_deferred'):
                try:
                    res = yield self.trap_deferred
                    if res and 'token_url' in res:
                        trap_url = res['token_url']
                except Exception as e:
                    log.msg(f"Trap token error: {e}")
            # new malware content
            trap_content = (
                f"#!/bin/bash\n"
            ...
            # temp file
            fd, trap_real_path = tempfile.mkstemp(prefix="cowrie_trap_")
            os.write(fd, trap_content)
            os.close(fd)
            ...
            self.fs.mkfile(
                ...
                self.currentlength, # we keep the original downloaded file length to be displayed upon checks
                33188,
            )
            
            self.fs.update_realfile(
                self.fs.getfile(self.outfile), trap_real_path # instead of self.artifact.shasumFilename
            )
            log.msg(f"wget: succesfully swapped {self.outfile} (simulating {self.currentlength} bytes) with CanaryToken")
 ...
 #%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def request_trap_token(self):
        """
        Canary Token parallel generation 
        ...
        # Filtering latest predictions
        valid_predictions = [p for p in raw_queue if now - p['timestamp'] < 60]
        
        # queue cleanup
        self.protocol.ai_prediction_queue = valid_predictions
        ...
        if active_count > 0:
            ...
            latency = now - best_pred['timestamp']
            
            other_msg = f" (+{active_count-1} others queued)" if active_count > 1 else ""
            
            memo = (f"PREDICTED THREAT MATCH \n"
                    f"URL: {target_url}\n"
                    f"Intent: {best_pred['intent']}\n"
                    f"Predicted path: {best_pred['predicted_path']}\n"
                    f"Latency: {latency:.2f}s{other_msg}")
            
            # empty queue
            self.protocol.ai_prediction_queue = []
        else:'''
        target_url = self.url.decode('utf-8', errors='ignore')
        memo = (f"Download from Standard Cowrie\n"
                f"URL: {target_url}")
        ...
        d = treq.post(API_URL, json=payload, timeout=5)
        ...
        d.addErrback(lambda _: None) # Silent fail
        return d
#%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
