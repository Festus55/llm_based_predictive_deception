# Scripts

Main scripts directory for the predictive honeypot system.

## Contents

- `sessions/` - Conversion scripts necessary to prepare the dataset, divide it into sessions and apply a rolling window pattern
- `batch_processing/` - Scripts for Google Cloud batch processing with Gemini
- `fine_tuning/` - Unsloth training scripts for Gemma 3 models
- `training_processing/` - Data preprocessing and formatting pipelines
